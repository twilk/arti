...
if (zmiany.length > 0) {
  powiedz(`Masz ${zmiany.length} ${zmiany.length === 1 ? 'zmieniony plik' : 'zmienionych plików'}. Zapisuję ${zmiany.length === 1 ? 'go' : 'je'} u Ciebie, zanim cokolwiek dociągnę.`);
  cicho('git', ['add', '-A']);
...
// Punkt powrotu bierze sie STAD, a nie sprzed zapisu powyzej. Za pierwszym razem
// staly te dwie linijki odwrotnie i wyszlo z tego cos gorszego niz zwykla pomylka:
// komunikat o konflikcie podsuwal `git reset --hard` na commit sprzed zapisu,
// czyli podpowiadal jej skasowanie dokladnie tej pracy, ktorej mial pilnowac.
...
if (!polaczenie.ok) {
  ...
  // Pytanie o kolidujace pliki musi paść PRZED cofnieciem - po `merge --abort`
  // nie ma juz czego pytac. Pierwsza wersja pytala o pliki rozne miedzy galeziami
  // i wymieniala takze te, ktore istnialy tylko po jednej stronie i z niczym sie
  // nie klocily - czyli wysylala ja z lista, na ktorej polowa pozycji byla falszywa.
  const kolidujace = (cicho('git', ['diff', '--name-only', '--diff-filter=U']) ?? '')
    .split('\n')
    .filter(Boolean)
    .slice(0, 5);
